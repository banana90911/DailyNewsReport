...
- 실제 앱 호스팅: Render(또는 VPS/EC2 등)

현재 저장소에는 Render 배포 설정이 포함되어 있습니다.
...
3. Render 환경변수 채우기 (특히 OAuth/OpenAI/Firecrawl)
4. GitHub Secrets 추가
...
- TTS 파일 경로는 `TTS_STORAGE_DIR`로 설정 가능 (기본 `storage/tts`)
- Firecrawl CLI 확인: `firecrawl --help`
- 디스코드 DM 전송은 사용자 Discord 연결이 필요합니다.
