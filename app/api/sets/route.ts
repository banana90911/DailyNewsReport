...
const createSetSchema = z.object({
  ...
  scheduleType: z.enum(["DAILY", "WEEKLY", "WEEKDAY", "WEEKEND", "CUSTOM"]),
...
export async function POST(request: Request) {
  ...
  const scheduleInput = {
    ...
    customDays: input.scheduleType === "CUSTOM" ? Array.from(new Set(input.customDays ?? [])) : [],
  ...
  const created = await prisma.categoryScheduleSet.create({
    data: {
      ...
      customDays: scheduleInput.customDays ?? [],
